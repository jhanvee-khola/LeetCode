...
class Solution{
  public:
    ...
    void clone(Node* root,unordered_map<Node*,Node*> &mp){
        if(!root){
            return;
        }
    ...
    void addClone(Node* root,unordered_map<Node*,Node*> &mp){
        if(!root){
            return;
        }
        Node *n=mp[root];
        n->left=mp[root->left];
        n->right=mp[root->right];
        n->random=mp[root->random];
        addClone(root->left,mp);
        addClone(root->right,mp);
    }
